...
def train_cmd(args):
    ...
    records,_,_,_=records_and_splits(args.data_dir); cfg=load_config(args.config); cfg.update({"seed":args.seed,"epochs":args.epochs,"evaluate_test":args.evaluate_test}); domains=[args.domain] if args.domain!="unified" else ["bacteria","archaea","eukaryota"]
    subset=[r for r in records if r.domain in domains and not r.partition.startswith("quarantine")]
    config_tag=Path(args.config).stem
    output=Path("results/checkpoints")/("unified" if args.domain=="unified" else args.domain)/args.model/config_tag/f"seed_{args.seed}"
    train(subset,args.model,output,cfg,not args.no_resume); print(output)
def compare(args):
    from src.evaluation import metrics
    roots=list((Path("results/checkpoints")/args.domain).glob("*/*/seed_*/validation_metrics.json")); rows=[]
    for p in roots: rows.append({"model":p.parents[2].name,"config":p.parents[1].name,"seed":p.parent.name,**json.loads(p.read_text())})
    Path("results/main_tables").mkdir(parents=True,exist_ok=True); out=Path("results/main_tables")/f"{args.domain}_comparison.json"; out.write_text(json.dumps(rows,indent=2)); print(out)
def predict_cmd(args):
    import torch
    from src.models import MODELS
    root=Path("results/final_models")/args.domain; meta=json.loads((root/"metadata.json").read_text()); threshold=json.loads((root/"threshold.json").read_text())["threshold"]; cfg=yaml.safe_load((root/"config.yaml").read_text()); model=MODELS[meta["model_name"]](**cfg); state=torch.load(root/"model.pt",map_location="cpu",weights_only=False); model.load_state_dict(state["model"] if "model" in state else state); model.eval()
    did={"bacteria":0,"archaea":1,"eukaryota":2}[args.domain]
...
def main():
    ...
    t=sub.add_parser("train"); t.add_argument("--data-dir",default=DEFAULT_DATA); t.add_argument("--domain",choices=["bacteria","archaea","eukaryota","unified"],required=True); t.add_argument("--model",choices=["residual_cnn","multiscale_cnn","cnn_bilstm","cnn_transformer","domain_aware"],required=True); t.add_argument("--config",default="configs/screening.yaml"); t.add_argument("--seed",type=int,default=2025); t.add_argument("--epochs",type=int,default=25); t.add_argument("--no-resume",action="store_true"); t.add_argument("--evaluate-test",action="store_true",help="Use only once after validation-only model selection is frozen"); t.set_defaults(func=train_cmd)
    c=sub.add_parser("compare"); c.add_argument("--domain",required=True); c.set_defaults(func=compare)
    q=sub.add_parser("predict"); q.add_argument("--domain",choices=["bacteria","archaea","eukaryota"],required=True); q.add_argument("--fasta",required=True); q.set_defaults(func=predict_cmd)
